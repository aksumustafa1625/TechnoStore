# Salesforce CLM Industries â€” Native Send for Signature: Signer Name Field Disabled, Cannot Send Envelope
...
We need to either (a) make the field editable / auto-populate, or (b) inject a Signer Name into the recipient via a server-side hook so the validation passes â€” without modifying the managed LWC, which we can't.
...
- The page is a managed Lightning Web Component (`runtime_industries_clm__contractSignatureView`). We do NOT have source access â€” it ships in the managed package.
...
- DocuSign integration in this org is fully working through a custom "Send Via DocuSign" quick action (envelope creation, email delivery, signed-status webhook) â€” proven end-to-end. So the underlying DocuSign auth/named credential / vendor account ID are correct.
...
1. Open a Contract â†’ click **Send for Signature** â†’ page loads correctly.
2. Click **Add Recipient**. The "Add Recipient" modal opens.
3. **Recipient Information** section â€” fields filled by typing/selecting:
   - Recipient Name: `Lukas MÃ¼ller` (a valid Contact lookup; pick from search).
   - Recipient Email: auto-populates as `lukas.mueller@technostore.example` from the contact (so the contact lookup is wired correctly).
...
4. **Signer Information** section:
   ...
   - **Signer Name: the input is disabled, shows the gray placeholder "Enter signer nameâ€¦", and accepts no keyboard input. The user cannot type into it.**
...
If we change Recipient Type from `Signer` to `Host In Person`, the Signer Name input becomes ACTIVE and accepts text. We typed `Lukas MÃ¼ller` into it and the row saved with the name visible in the Signer Name column. But that recipient counts as a "host" in DocuSign â€” not a remote-email signer â€” so it doesn't satisfy the demo (no email goes to the customer; it's an in-person kiosk flow).
...
| API Name | Type | Notes |
|---|---|---|
| `RecipientName` | string | The display name of the contact (e.g. "Lukas MÃ¼ller"). |
...
- `ESignatureConfig` has 3 records:
  - `SignerRoles` / `AnchorTabSetting` â€” value: `Customer Signer,Internal Approver,Witness,CC Recipient`
  - `CalloutNamedCredential` / `CalloutConfigurationSetup` â€” value: `DocuSign`
  - `eSignVendorAccountId` / `eSignVendorAccount` â€” value: `47481170`
...
### Attempt 1 â€” Apex `before insert / before update` trigger on DocumentRecipient
...
Deploy succeeded. We re-attempted the Send â€” same error. After the failed Send, we queried `DocumentRecipient` and found **zero rows**. **Conclusion**: the LWC validates in-memory before any DML. The trigger never fires because the insert never happens.

### Attempt 2 â€” Verify the linked Contact has all expected fields

Maybe the auto-population needs FirstName + LastName + Name to all be populated:

```sql
SELECT Id, FirstName, LastName, Name, Email FROM Contact WHERE Email='lukas.mueller@technostore.example'
```

Returned `Lukas / MÃ¼ller / Lukas MÃ¼ller / lukas.mueller@technostore.example`. All standard fields present, formula `Name` resolved correctly. Still no auto-population.

### Attempt 3 â€” Inspect ESignatureConfig for a field-mapping setting

Three records, listed above. None of them looks like a "RecipientFieldDefaults" / "SignerNameFromContactField" mapping. We searched for the string `LegalName` across `EntityParticle` â€” only one entity has it (`DocumentRecipient`), and there is no apparent CLM config that maps a Contact field to it.

### Attempt 4 â€” Look for any published API hook or "before-send" flow trigger surface
...
### Attempt 5 â€” Click directly into the "Enter signer name..." input and try to type
...
- **Modify the LWC**: not possible â€” it's managed.
